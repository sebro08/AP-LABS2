  ...
  const aprobarSolicitud = async (solicitud: SolicitudGestion) => {
    ...
    try {
      ...
      if (solicitud.tipo === 'LABORATORIO') {
        ...
        // Crear notificación para el usuario
        const fechaInicio = datosRecurso.fecha_reserva ? new Date(datosRecurso.fecha_reserva).toLocaleDateString('es-ES') : 'N/A';
        const fechaFin = datosRecurso.fecha_devolucion ? new Date(datosRecurso.fecha_devolucion).toLocaleDateString('es-ES') : 'N/A';
        await crearNotificacion(
...
export default TecnicoGestionSolicitudes;
